...
| **emotion-awareness type** | derivable (S1) | **Drop the direct ask.** `emotional_metadata.specificity` *is* this scale and the first session measures it better than a self-report. At most, defer one lightweight self-rating later to validate the signal. |
| **talker vs not-a-talker** | derivable proxy (S1-2) | **Defer / drop.** `rawInputLength`, `inputDuration`, `say_more` usage, `preferredInputType` cover it immediately. If kept, fold into the intention question, not standalone. |
