...
- **The `reaching` mirror ends on a question instead of stopping** — previously, when a mirror named a gap it couldn't close, it just stopped there, reading like it had given up. It now closes the shortfall in one continuous utterance that ends on a question. Whether the question can name something specific is resolved by prompt structure, not new code: `buildMemoryContext` only renders the semantic-profile heading when a profile exists, so the model conditions on whether that section is present — profile present, the question names one thing from it; cold start, it proposes nothing. This is a scoped reversal of the standing "declarative, never interrogative" constraint (#171), for this block only. No new field, claim strength, or gate signal — `routeClaimStrength`, `decideMirrorOutcome`, and `holding` are untouched.
