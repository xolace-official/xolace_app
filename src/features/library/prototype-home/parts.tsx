...
export const openEntry = (_slug: string) =>
  router.push("/library-reader-prototype?variant=A" as never);
/** Reader href carrying this entry's cover + title, so a zoom lands on the same photo (#396). */
export const readerHref = (e: Entry) =>
  ({
    pathname: "/library-reader-prototype",
    params: { variant: "A", cover: e.cover, title: e.title },
  }) as never;
export const openList = (q: string) =>
  router.push(`/browse/library-prototype/list?${q}` as never);
export const openSubjects = (letter?: string) =>
  router.push(
    `/browse/library-prototype/subjects${letter ? `?letter=${letter}` : ""}` as never,
  );
...
export function Meta({
  ...
  return (
    <View className="flex-row flex-wrap items-center gap-1.5">
      <AppText
        className={
          onImage ? "text-[13px] text-white/75" : "text-muted text-[13px]"
        }
      >
        ...
        <View
          className={
            onImage
              ...
              : "bg-accent/15 rounded-full px-1.5"
          }
        >
          <AppText
            className={
...
export function UpNextCard({
  ...
  return (
    // Zoom experiment (#396): the card's face flies into the reader cover.
    // Pressable stays outermost so the card remains one accessible link;
    // AppleZoom around the Pressable hid it from VoiceOver.
    <Link href={readerHref(entry)} asChild>
        ...
        <Link.AppleZoom>
          ...
          >
            ...
            <View className="flex-1 justify-end gap-2 p-5">
              <AppText className="text-[12px] font-medium uppercase tracking-[1.5px] text-white/70">
                {kicker}
              </AppText>
              <AppText
                style={[SERIF, { fontSize: 26, lineHeight: 31 }]}
...
export function EntryRow({
  ...
  // Zoom experiment (#396): only the thumbnail flies into the reader cover; the text stays put.
  return (
    <Link href={readerHref(entry)} asChild>
      <Pressable className="flex-row items-center gap-4 px-4 py-3 active:opacity-70">
        <Link.AppleZoom>
          <Image source={{ uri: entry.cover }} style={THUMB} transition={200} />
        </Link.AppleZoom>
        <View className="flex-1 gap-1">
          <AppText className="text-muted text-[12px]">
            {reason ? `${reason} · ` : ""}
            {kindLabel(entry.kind)} · {entry.subject}
          </AppText>
          <AppText
...
// One flat object: Link.AppleZoom clones its child and rejects style arrays
const THUMB = { width: 104, height: 78, borderRadius: 14 };
    ...
    <View className="flex-row items-end justify-between px-4 pb-3 pt-8">
      ...
      {action && (
        <Pressable onPress={onAction} hitSlop={8}>
          <AppText className="text-accent text-[15px] font-medium">
            {action}
          </AppText>
        </Pressable>
      )}
    ...
    <View className="bg-surface mx-4 mt-4 gap-3 rounded-[24px] p-4">
      <View className="flex-row items-start justify-between">
        <View className="flex-1 gap-0.5">
          ...
          <AppText className="text-muted text-[14px]">
            Pick any that fit. It only shapes what’s picked for you.
          </AppText>
        </View>
        ...
        {AUDIENCES.map((a) => {
          ...
          return (
            <Pressable
              ...
              className={
                on
                  ? "bg-accent rounded-full px-3.5 py-2"
                  : "bg-surface-secondary rounded-full px-3.5 py-2"
              }
            >
              <AppText
                className={
                  on
                    ? "text-accent-foreground text-[14px] font-medium"
      ...
      {audiences.length > 0 && (
        <Pressable
          onPress={onDone}
          className="bg-foreground items-center rounded-full py-2.5"
        >
          <AppText className="text-background text-[15px] font-semibold">
            Done
          </AppText>
        </Pressable>
      )}
