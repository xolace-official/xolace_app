...
import { SERIF, openList } from './parts';
...
function Dot({ index, scrollX, page, color }: { index: number; scrollX: SharedValue<number>; page: number; color: string }) {
  ...
  return <Animated.View style={[{ height: DOT, borderRadius: DOT / 2, backgroundColor: color, borderCurve: 'continuous' }, style]} />;
}
  ...
  return (
    <View>
      <ScrollShadow LinearGradientComponent={LinearGradient} size={28}>
        ...
        >
          {hubs.map((hub) => (
            <Pressable key={hub.slug} onPress={() => openList(`hub=${hub.slug}`)} className="bg-surface rounded-[28px] p-2 active:opacity-90" style={{ width: cardW, borderCurve: 'continuous' }}>
      ...
      <View className="flex-row items-center justify-center pt-4" style={{ gap: 8 }}>
        {hubs.map((h, i) => <Dot key={h.slug} index={i} scrollX={scrollX} page={page} color={accent} />)}
      </View>
    </View>
  );
}
