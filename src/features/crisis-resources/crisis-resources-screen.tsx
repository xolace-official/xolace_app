...
import { PressableFeedback, Select, useThemeColor, useToast } from 'heroui-native';
...
export function CrisisResourcesScreen() {
  ...
  const handleCountryChange = (option: SelectOption) => {
    ...
    setCountry(code && COUNTRY_RESOURCES[code as CountryCode] ? (code as CountryCode) : null);
  };
  ...
  const handleEmergencyCall = async () => {
    ...
    await Linking.openURL(url).catch(() => {
      toast.show({ label: 'Unable to open dialer. Please dial manually.', variant: 'default' });
    });
    try { posthog.capture('emergency_call_tapped', { country: selectedCountry }); } catch {}
  };

  const handleResourceTap = (resource: { type: string; source: string; label: string; value: string }) => {
    try {
