...
// PROTOTYPE — throwaway. Card silhouettes for the founder-message marquee.
// Feedback (#236): no shape that eats the photo (no pointed ovals, no crescent);
// `flower` from the biscuit-camera sample is wanted; one card per shape, no
// duplicates. Five shapes, five cards.

export type FounderCardShape = "circle" | "roundedRect" | "portal" | "blob" | "flower";

export const FOUNDER_CARD_SHAPES: readonly FounderCardShape[] = [
  "circle",
  "roundedRect",
  "portal",
  "blob",
...
const IMAGE_CORNER_RADIUS = 20; // from the biscuit-camera sample
const FLOWER_LOBE_STEPS = 108; // 12×9 — even point density per lobe

/** Inscribed circle filling the inner rect (biscuit-camera sample). */
...
/** Soft rounded rectangle (biscuit-camera sample). */
...
/** Scalloped rose / flower — nine shallow lobes (biscuit-camera sample). */
const flowerPath = (w: number, h: number): SkPath => {
  ...
  const rh = h / 2;
  const lobeCount = 9;
  const amplitude = 0.18; // shallow scallops so the photo stays readable
